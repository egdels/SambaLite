...
/**
 * WorkManager Worker that performs folder synchronization in the background.
 * Iterates over all enabled SyncConfigs and syncs each folder pair.
 */
public class FolderSyncWorker extends Worker {
    ...
    public Result doWork() {
        ...
        ConnectionRepositoryImpl connectionRepository = new ConnectionRepositoryImpl(getApplicationContext());
        ...
        for (SyncConfig config : configs) {
            ...
            if (specificConfigId == null) {
                ...
                if (lastSync > 0 && elapsed < intervalMs) {
                    LogUtils.d(TAG, "Skipping config " + config.getId()
                            + " (next sync in " + ((intervalMs - elapsed) / 60000) + " min)");
                    continue;
                }
            }
    ...
    private void syncFolder(SyncConfig config, SmbConnection connection) throws Exception {
        ...
        DocumentFile localFolder = DocumentFile.fromTreeUri(getApplicationContext(),
                Uri.parse(config.getLocalFolderUri()));
    ...
    private void syncRemoteToLocal(DiskShare share, DocumentFile localFolder, String remotePath) {
        ...
        try {
            ...
            for (FileIdBothDirectoryInformation remoteFile : remoteFiles) {
                ...
                String remoteFilePath = smbJoin(remotePath, name);
                boolean isDirectory = (remoteFile.getFileAttributes()
     ...
     */
    private void uploadFile(DiskShare share, DocumentFile localFile, String remotePath) throws Exception {
        LogUtils.d(TAG, "Uploading: " + localFile.getName() + " -> " + remotePath);

        try (File remoteFile = share.openFile(remotePath,
                EnumSet.of(AccessMask.GENERIC_WRITE),
                EnumSet.of(FileAttributes.FILE_ATTRIBUTE_NORMAL),
                SMB2ShareAccess.ALL,
                SMB2CreateDisposition.FILE_OVERWRITE_IF, null);
             InputStream is = getApplicationContext().getContentResolver().openInputStream(localFile.getUri());
     ...
     */
    private void downloadFile(DiskShare share, String remotePath, DocumentFile localFile) throws Exception {
        ...
        try (File remoteFile = share.openFile(remotePath,
             ...
             OutputStream os = getApplicationContext().getContentResolver().openOutputStream(localFile.getUri())) {
    ...
    private void setRemoteFileLastModified(DiskShare share, String remotePath, long timeMillis) {
        try (File remoteFile = share.openFile(remotePath,
            ...
            FileBasicInformation newInfo = new FileBasicInformation(
            ...
            LogUtils.w(TAG, "Could not set last modified time for: " + remotePath + ": " + e.getMessage());
        }
    }
    ...
    private long getRemoteFileSize(DiskShare share, String remotePath) {
        try (File remoteFile = share.openFile(remotePath,
                ...
                SMB2ShareAccess.ALL,
                SMB2CreateDisposition.FILE_OPEN, null)) {
            return remoteFile.getFileInformation()
    ...
    private long getRemoteFileLastModified(DiskShare share, String remotePath) {
        try (File remoteFile = share.openFile(remotePath,
                ...
                SMB2CreateDisposition.FILE_OPEN, null)) {
            return remoteFile.getFileInformation()
                    .getBasicInformation().getLastWriteTime().toEpochMillis();
        } catch (Exception e) {
