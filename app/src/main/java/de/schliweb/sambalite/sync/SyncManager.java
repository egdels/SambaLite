...
/**
 * Manages folder synchronization scheduling and configuration.
 * Coordinates between SyncRepository (persistence) and WorkManager (scheduling).
 */
@Singleton
public class SyncManager {
     ...
     */
    public SyncConfig addSyncConfig(String connectionId, Uri localFolderUri,
                                    String remotePath, String localFolderDisplayName,
                                    SyncDirection direction, int intervalMinutes) {
        LogUtils.d(TAG, "Adding sync config for connection: " + connectionId);

        // Take persistable URI permission
        try {
            context.getContentResolver().takePersistableUriPermission(localFolderUri,
    ...
    public void schedulePeriodicSync() {
        ...
        PeriodicWorkRequest request = new PeriodicWorkRequest.Builder(
                FolderSyncWorker.class, minInterval, TimeUnit.MINUTES)
                .setConstraints(constraints)
