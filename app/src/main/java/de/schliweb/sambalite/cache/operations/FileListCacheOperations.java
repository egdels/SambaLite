...
/**
 * Handles file list specific caching operations. This class centralizes the file list caching logic
 * that was previously scattered throughout the original IntelligentCacheManager.
 */
public class FileListCacheOperations {
  ...
  public void cacheFileList(
    ...
    LogUtils.d(
        TAG,
        "Created files copy of type: "
            + filesCopy.getClass().getSimpleName()
            + " with "
            + filesCopy.size()
            + " items");
    ...
    if (entry != null) {
      ...
      if (entry.getData() instanceof List) {
        try {
          ...
          LogUtils.d(
              TAG,
              "Retrieved cached file list for key: " + cacheKey + " (" + files.size() + " files)");
  ...
  public void invalidateAllFileLists(@NonNull SmbConnection connection) {
    ...
    LogUtils.d(
        TAG, "Invalidated " + count + " file list caches for connection: " + connection.getName());
  }

  /**
   * Preloads common file lists for a specific connection and path. This can be used to improve
   * performance by caching file lists that are likely to be accessed soon.
   *
   * @param connection The SMB connection
   * @param path The path to the directory
   */
  public void preloadCommonFileLists(@NonNull SmbConnection connection, @NonNull String path) {
    ...
    LogUtils.d(
        TAG,
        "Preloading common file lists for connection: " + connection.getName() + ", path: " + path);
  }
}
