...
/**
 * A hybrid cache strategy that combines memory and disk caching. This strategy first checks the
 * memory cache, then falls back to the disk cache if not found. It provides the best balance
 * between performance and persistence.
 *
...
public class HybridCacheStrategy<K, V> implements CacheStrategy<K, V> {
  ...
  @Getter private final MemoryCacheStrategy<K, V> memoryStrategy;

  // The disk cache strategy
  @Getter private final DiskCacheStrategy<K, V> diskStrategy;
  ...
  public CacheEntry<V> get(K key) {
    ...
    if (entry != null) {
      ...
      LogUtils.d(
          TAG,
          "Cache hit in memory for key: " + key + ", total hits: " + statistics.getCacheHits());
      return entry;
    }
    ...
    if (entry != null) {
      ...
      LogUtils.d(
          TAG,
          "Cache hit on disk for key: "
              + key
              + ", stored in memory cache, total hits: "
    ...
    LogUtils.d(
        TAG,
        "Cache miss in hybrid cache for key: "
            + key
            + ", total misses: "
  ...
  public int removePattern(String keyPattern) {
    ...
    LogUtils.d(
        TAG,
        "Removed " + memoryCount + " entries from memory and " + diskCount + " entries from disk");
  ...
  /**
   * Logs detailed cache statistics for debugging purposes. This provides a comprehensive view of
   * the cache state.
   */
  private void logCacheStatistics() {
    ...
    // Error statistics
    int totalErrors =
        statistics.getSerializationErrors()
            + statistics.getDeserializationErrors()
            + statistics.getDiskWriteErrors()
            + statistics.getDiskReadErrors();
    ...
    if (totalErrors > 0) {
      ...
      stats
          ...
          .append(statistics.getSerializationErrors())
          .append("\n");
      stats
          .append("- Deserialization Errors: ")
          .append(statistics.getDeserializationErrors())
          .append("\n");
