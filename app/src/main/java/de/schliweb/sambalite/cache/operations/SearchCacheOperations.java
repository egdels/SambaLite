...
/**
 * Handles search result specific caching operations. This class centralizes the search result
 * caching logic that was previously scattered throughout the original IntelligentCacheManager.
 */
public class SearchCacheOperations {
  ...
  public void cacheSearchResults(
    ...
    // Generate cache key
    String cacheKey =
        keyGenerator.generateSearchKey(connection, path, query, searchType, includeSubfolders);
    ...
    LogUtils.d(
        TAG,
        "Created results copy of type: "
            + resultsCopy.getClass().getSimpleName()
            + " with "
            + resultsCopy.size()
            + " items");
    ...
    LogUtils.d(
        TAG,
        "Successfully cached search results for key: " + cacheKey + " with TTL: " + ttl + "ms");
  }
  ...
  public @NonNull List<SmbFileItem> getCachedSearchResults(
    ...
    LogUtils.d(
        TAG,
        "Getting cached search results for connection: "
            + connection.getName()
            + ", path: "
            + path
            + ", query: "
    ...
    if (entry != null) {
      ...
      if (entry.getData() instanceof List) {
        try {
          ...
          LogUtils.d(
              TAG,
              "Retrieved cached search results for key: "
                  + cacheKey
                  + " ("
                  + results.size()
                  + " results)");
        ...
        LogUtils.e(
            TAG,
            "Cache entry is not a List for key: "
                + cacheKey
                + ", type: "
                + (entry.getData() != null ? entry.getData().getClass().getSimpleName() : "null"));
      }
    }
  ...
  /**
   * Invalidates the search cache for a specific connection and path. This should be called when
   * files are added, deleted, or modified in the directory.
   *
   * @param connection The SMB connection
   * @param path The path to the directory
   */
  public void invalidateSearchCache(@NonNull SmbConnection connection, @NonNull String path) {
    ...
    String pattern =
        "search_conn_" + connection.getId() + "_path_" + keyGenerator.sanitizePath(path);
    ...
    LogUtils.d(
        ...
        "Invalidated "
            + count
            + " search cache entries for connection: "
            + connection.getName()
            + ", path: "
            + path);
  }
  ...
  public void invalidateAllSearchCaches(@NonNull SmbConnection connection) {
    ...
    LogUtils.d(
        TAG,
        "Invalidated " + count + " search cache entries for connection: " + connection.getName());
  }

  /**
   * Preloads common search queries for a specific connection and path. This can be used to improve
   * performance by caching search results that are likely to be accessed soon.
   *
   * @param connection The SMB connection
   * @param path The path to the directory
   */
  public void preloadCommonSearches(@NonNull SmbConnection connection, @NonNull String path) {
    ...
    LogUtils.d(
        TAG,
        "Preloading common searches for connection: " + connection.getName() + ", path: " + path);
  }
