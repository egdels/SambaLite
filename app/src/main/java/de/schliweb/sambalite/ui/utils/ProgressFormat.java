...
public final class ProgressFormat {
  ...
  public static @NonNull Result parse(
    ...
    String finalName = base != null && !base.isEmpty() ? base : baseFallback;
  ...
  private static final Pattern P_PCT_IDX_NAME =
      Pattern.compile("^(\\d{1,3})%\\s*\\((\\d+)\\s*/\\s*(\\d+)\\)\\s*-\\s*(.+)$");
  ...
  private static final Pattern P_BYTES_ONLY =
      Pattern.compile(
          "^\\s*([0-9]+(?:\\.[0-9])?\\s*[KMGTPE]?B)\\s*/\\s*([0-9]+(?:\\.[0-9])?\\s*[KMGTPE]?B)\\s*$",
          Pattern.CASE_INSENSITIVE);

  private static final Pattern P_ANY_PERCENT = Pattern.compile("(\\d{1,3})%");
