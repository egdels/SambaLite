 ...
 * handles all UI elements related to progress feedback and provides a unified approach to user
 * feedback across the application.
 *
 * <p>It implements both ProgressCallback for backward compatibility and UserFeedbackProvider for
 * the new consolidated approach to user feedback.
 */
public class ProgressController implements ProgressCallback, UserFeedbackProvider {
        ...
        () -> {
          try {
            ...
            builder.setNegativeButton(
                ...
                (dialog, which) -> {
                  LogUtils.d(
                      "ProgressController", "User requested cancellation from progress dialog");
  ...
  public void updateDetailedProgress(
    ...
    activity.runOnUiThread(
        () -> {
          ...
          LogUtils.d(
              "ProgressController",
              "Progress updated: overall="
                  + overallPct
                  + "%, idx="
                  + (cur > 0 ? cur : lastCur)
                  + "/"
                  + (total > 0 ? total : lastTotal)
                  + ", filePct="
                  + filePct
                  + ", name="
                  + baseName);
  ...
  /**
   * Sets a cancel action for the detailed progress dialog. This should be called immediately after
    ...
    activity.runOnUiThread(
        () -> {
          try {
            ...
            LogUtils.e(
                "ProgressController",
                "Error setting cancel action for detailed progress dialog: " + e.getMessage());
          }
        });
  }
  ...
  public void showFileExistsDialog(
    ...
    if (!isActivitySafe()) {
      LogUtils.w(
          "ProgressController",
          "Activity is finishing/destroyed, cancelling file exists dialog for: " + fileName);
      cancelAction.run();
      return;
    }
            ...
            new MaterialAlertDialogBuilder(activity)
                ...
                .setNegativeButton(
                    ...
                    (dialog, which) -> {
                      ...
                      LogUtils.d(
                          "ProgressController",
                          "User cancelled file exists dialog for: " + finalFileName);
                      finalCancelAction.run();
                    })
                .setCancelable(false)
                .show();
  ...
  public void showConfirmation(
    ...
    if (!isActivitySafe()) {
      LogUtils.w(
          "ProgressController", "Activity is finishing/destroyed, cancelling confirmation dialog");
      if (onCancel != null) {
        onCancel.run();
      }
      return;
    ...
    activity.runOnUiThread(
        () -> {
          try {
            ...
            LogUtils.e(
                "ProgressController", "Error showing confirmation dialog: " + e.getMessage());
