...
/**
 * Unified performance monitor for SambaLite. Tracks basic and advanced performance metrics
 * depending on the operation type. Consolidates functionality from both SimplePerformanceMonitor
 * and AdvancedPerformanceMonitor.
 */
public class SimplePerformanceMonitor {
  ...
  public static void endOperation(@NonNull String operationName) {
    ...
    if (isUIOperation(operationName)) {
      ...
      LogUtils.i(
          TAG,
          "Completed operation: " + operationName + " in " + duration + "ms (" + performance + ")");
  ...
  private static void startAdvancedOperation(String operationName) {
    ...
    LogUtils.d(
        TAG,
        "Starting advanced operation: "
            + operationName
            + " | Memory: "
            + EnhancedFileUtils.formatFileSize(memoryUsage)
  ...
  private static void endAdvancedOperation(String operationName) {
    ...
    LogUtils.i(
        ...
        "Advanced operation completed: "
            ...
            + performanceLevel
            + " | Memory Delta: "
            + EnhancedFileUtils.formatFileSize(memoryDelta)
            + " | Final Memory: "
            + EnhancedFileUtils.formatFileSize(endMemory));
  ...
  public static @NonNull String getMemoryInfo() {
    ...
    return String.format(
        Locale.US,
        "Memory: %s used / %s max (%.1f%% usage)",
        EnhancedFileUtils.formatFileSize(usedMemory),
        EnhancedFileUtils.formatFileSize(maxMemory),
  ...
  public static @NonNull String getPerformanceStats() {
    ...
    return String.format(
        Locale.US,
        "Performance: %d operations, avg %.1fms, total %.1fs",
        count,
        average,
        total / 1000.0);
  }
  ...
  /**
   * Determines if this is a UI operation (simple monitoring) or cache/network operation (advanced
   * monitoring).
   */
  private static boolean isUIOperation(String operationName) {
    return operationName.contains("Activity")
        || operationName.contains("onCreate")
        || operationName.contains("fileClick")
        || operationName.contains("connectionClick");
  }
