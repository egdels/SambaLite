 ...
 *
 * <p>Key improvements vs. previous implementation: - Reference-counted WakeLock to avoid premature
 * release during concurrent ops - Throttled notification updates to bypass rate limiting - Watchdog
 * with inactivity + absolute timeout and cooperative cancellation flag - Helper APIs for temp files
 * + atomic finalize (temp ➜ destination) - Operation-specific notification intents (tap to open
 * relevant screen/path)
 */
public class SmbBackgroundService extends Service {
  ...
  private final java.util.concurrent.ConcurrentLinkedQueue<Future<?>> runningFutures =
  ...
  private final android.os.Handler notificationHandler =
      new android.os.Handler(android.os.Looper.getMainLooper());

  // State for notification texts/intents
  private volatile String currentOperation = "";
  private volatile boolean isRunning = false;
  @Getter private volatile boolean stopRequested = false;
  ...
  public void startOperation(@NonNull String operationName) {
    ...
    boolean contextChanged =
        (prevSearch != isSearchOperation)
            || (prevUpload != isUploadOperation)
            || (prevDownload != isDownloadOperation);

    currentOperation = title;
    LogUtils.d(TAG, "Starting operation: " + operationName + " (active=" + count + ")");

    // If the context changed (e.g., Search -> Download), refresh immediately so the tap target
    // updates now.
    if (contextChanged) {
  ...
  public void updateFileProgress(
    ...
    String title = op.label() + "…";
    String info =
        ProgressFormat.buildUnified(
            op, currentFile, totalFiles, currentFileName != null ? currentFileName : "");
  ...
  public void updateBytesProgress(
    ...
    String title = op.label() + "…";
    String info =
        ProgressFormat.formatBytes(op.label(), currentBytes, totalBytes)
  ...
  public void setDownloadParameters(@NonNull String connectionId, @NonNull String downloadPath) {
    ...
    this.isUploadOperation = false;
    LogUtils.d(
        TAG, "Download parameters set: connectionId=" + connectionId + ", path=" + downloadPath);
        ...
        () -> {
          ...
          if (hasActiveOperations() && idle > PROGRESS_WATCHDOG_INTERVAL_SECONDS * 1000L) {
            ...
            updateNotificationImmediate(
                "Operation timed out", "No progress for " + (idle / 1000) + "s, cancelling...");
          }
        };
  ...
  public @NonNull File createTempFile(@NonNull String originalFileName) throws IOException {
    ...
    String base =
        (originalFileName == null ? "transfer" : originalFileName).replaceAll("[^a-zA-Z0-9]", "_");
  ...
  private void cancelAllOperations(String reason) {
    ...
    updateNotificationImmediate("Operation cancelled", reason != null ? reason : "Cancelled");
        ...
        () -> {
          if (hasActiveOperations() && lastProgressUpdate.get() > 0) {
            ...
            if (idle > PROGRESS_WATCHDOG_INTERVAL_SECONDS * 1000L) {
              LogUtils.w(
                  TAG, "Watchdog: stalled op (" + (idle / 1000) + "s) — setting cancel flag");
              updateNotificationImmediate(
                  "Operation timed out",
