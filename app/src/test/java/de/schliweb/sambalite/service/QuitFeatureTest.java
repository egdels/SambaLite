 ...
 *
 * 1. BackgroundSmbManager:
 *    - hasActiveOperations / getActiveOperationCount (with and without service)
 *    - requestStopService (unbind, stopRequested flag, ACTION_STOP intent)
 *    - ensureServiceRunning (resets stopRequested, restarts service)
 *    - onServiceDisconnected behavior (auto-restart vs. no-restart after stop)
 *    - onServiceConnected mirrors stopRequested from service
 *
 * 2. SmbBackgroundService:
 *    - ACTION_STOP handling (stops service, cancels operations, sets flags)
 *    - ACTION_STOP with active operations (all cancelled)
 *    - ACTION_CANCEL handling
 *    - stopRequested flag lifecycle
 *    - Normal startup resets stopRequested
 *    - Notification stop action shown when idle
...
public class QuitFeatureTest {
    ...
    /**
     * Creates a BackgroundSmbManager with a mock context, captures the ServiceConnection,
     * and optionally connects a real Robolectric service.
     */
    ...
    public void requestStopService_afterStop_serviceNotReconnected() {
        ...
        verify(mockAppContext, never()).bindService(any(Intent.class), any(ServiceConnection.class), anyInt());
    }
    ...
    public void ensureServiceRunning_afterStop_restartsService() {
        ...
        when(mockAppContext.getApplicationContext()).thenReturn(mockAppContext);
        doReturn(null).when(mockAppContext).startForegroundService(any());
        doReturn(true).when(mockAppContext).bindService(any(Intent.class), any(ServiceConnection.class), eq(Context.BIND_AUTO_CREATE));

        // ensureServiceRunning should restart the service
        mgr.ensureServiceRunning();

        verify(mockAppContext).startForegroundService(any());
        verify(mockAppContext).bindService(any(Intent.class), any(ServiceConnection.class), eq(Context.BIND_AUTO_CREATE));
    }
    ...
    public void onServiceConnected_mirrorsStopRequestedFromService() {
        ...
        assertTrue("Service should have stopRequested=true after ACTION_STOP", service.isStopRequested());
