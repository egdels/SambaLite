...
public class SmbRepositoryIntegrationTest {
  ...
  public void testRepositoryListFiles() throws Exception {
    ...
    System.out.println(
        "[DEBUG_LOG] Repository list files test successful: " + result.size() + " items");
  }
  ...
  public void testRepositoryDownloadFile() throws Exception {
    ...
    doNothing()
        .when(mockSmbRepository)
        .downloadFile(eq(testConnection), eq(testFilePath), any(java.io.File.class));
  ...
  public void testRepositoryUploadFile() throws Exception {
    ...
    doNothing()
        .when(mockSmbRepository)
        .uploadFile(eq(testConnection), any(java.io.File.class), eq(testFilePath));

    // Test repository call
    mockSmbRepository.uploadFile(testConnection, tempFile, testFilePath);

    // Verify mock was called
    verify(mockSmbRepository, times(1))
        .uploadFile(eq(testConnection), any(java.io.File.class), eq(testFilePath));
  ...
  public void testRepositoryCreateDirectory() throws Exception {
    ...
    doNothing()
        .when(mockSmbRepository)
        .createDirectory(eq(testConnection), eq(testDirPath), eq(testDirName));
  ...
  public void testRepositoryMultipleOperations() throws Exception {
    ...
    doNothing()
        .when(mockSmbRepository)
        .createDirectory(eq(testConnection), eq(basePath), eq(dirName));
    doNothing()
        .when(mockSmbRepository)
        .uploadFile(eq(testConnection), any(java.io.File.class), eq(basePath + "/test.txt"));
    doNothing()
        .when(mockSmbRepository)
        .downloadFile(eq(testConnection), eq(basePath + "/test.txt"), any(java.io.File.class));
  ...
  public void testRepositoryConnectionPooling() throws Exception {
    ...
    when(mockSmbRepository.listFiles(eq(connection1), eq("/")))
        .thenReturn(testHelper.listFiles("/"));
    when(mockSmbRepository.listFiles(eq(connection2), eq("/")))
        .thenReturn(testHelper.listFiles("/"));
  ...
  public void testRepositoryPerformanceMetrics() throws Exception {
    ...
    when(mockSmbRepository.listFiles(eq(testConnection), eq("/")))
        .thenAnswer(
            invocation -> {
