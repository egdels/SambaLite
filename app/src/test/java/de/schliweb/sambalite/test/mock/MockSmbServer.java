...
/**
 * Mock SMB server for unit testing. Simulates SMB file operations in memory without requiring
 * network connectivity.
 *
 * <p>This mock server provides: - In-memory file system simulation - Directory listing
 * functionality - File upload/download operations - Connection management simulation - Error
 * injection for testing edge cases
 */
public class MockSmbServer {
  ...
  public void uploadFile(MockConnection connection, String filePath, InputStream inputStream)
      throws IOException {
    simulateNetworkDelay();
  ...
  private boolean isDirectChild(String parentPath, String childPath) {
    if (parentPath.equals("/")) {
      ...
      return childPath.startsWith("/")
          && childPath.substring(1).indexOf('/') == -1
          && !childPath.equals("/");
    } else {
  ...
  private SmbFileItem createFileItem(String name, String path, long size, long lastModified) {
    return new SmbFileItem(
        name, path, SmbFileItem.Type.FILE, size, new java.util.Date(lastModified));
  }

  private SmbFileItem createDirectoryItem(String name, String path) {
    return new SmbFileItem(
        name, path, SmbFileItem.Type.DIRECTORY, 0, new java.util.Date(System.currentTimeMillis()));
  }
  ...
  public MockServerStats getStats() {
    return new MockServerStats(
        directories.size(), files.size(), connections.size(), calculateTotalFileSize());
  }
  ...
  public static class MockServerStats {
    ...
    public MockServerStats(
        int directoryCount, int fileCount, int connectionCount, long totalFileSize) {
    ...
    public String toString() {
      return String.format(
          "MockServerStats{dirs=%d, files=%d, connections=%d, totalSize=%d}",
          directoryCount, fileCount, connectionCount, totalFileSize);
    }
  }
}
