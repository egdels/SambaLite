...
/**
 * Comprehensive SMB integration tests using the mock SMB server. Tests all major SMB operations
 * including file listing, upload, download, and error scenarios.
 */
public class SmbIntegrationTest {
  ...
  public void setUp() {
    ...
    // Use mock-only mode: these tests rely on mock-specific behavior
    // (default test data content, error injection, network delay simulation)
  ...
  public void testListSubdirectory() {
    ...
    System.out.println(
        "[DEBUG_LOG] Documents directory listing successful: " + items.size() + " items");
  }
    ...
    try {
      ...
      for (SmbFileItem item : items) {
        if ("uploaded_test.txt".equals(item.getName())) {
          ...
          assertEquals(
              "Uploaded file should have correct size", testContent.length(), item.getSize());
          break;
        }
      }
  ...
  public void testDeleteFile() {
    try {
      ...
      boolean fileExistsBefore =
          itemsBefore.stream().anyMatch(item -> "delete_test.txt".equals(item.getName()));
      ...
      boolean fileExistsAfter =
          itemsAfter.stream().anyMatch(item -> "delete_test.txt".equals(item.getName()));
  ...
  public void testErrorScenarios() {
    ...
    try {
      ...
      assertTrue(
          "Should get connection failure exception", e.getMessage().contains("connection failure"));
    }
    ...
    try {
      ...
      assertTrue(
          "Should get file operation failure exception",
  ...
  public void testFileNotFound() {
    try {
      ...
      assertTrue("Should get file not found exception", e.getMessage().contains("not found"));
    }
    ...
    try {
      ...
      for (SmbFileItem item : items) {
        if ("large_test_file.bin".equals(item.getName())) {
          fileFound = true;
          assertEquals("Large file should have correct size", largeContent.length, item.getSize());
          break;
        }
      }
      ...
      while ((bytesRead =
              downloadStream.read(
                  downloadedContent, totalBytesRead, downloadedContent.length - totalBytesRead))
          != -1) {
        totalBytesRead += bytesRead;
      }

      assertEquals(
          "Downloaded size should match uploaded size", largeContent.length, totalBytesRead);

      // Verify content integrity
      for (int i = 0; i < largeContent.length; i++) {
        assertEquals(
            "Byte at position " + i + " should match", largeContent[i], downloadedContent[i]);
      }

      downloadStream.close();

      System.out.println(
          "[DEBUG_LOG] Large file operations successful: " + totalBytesRead + " bytes");
