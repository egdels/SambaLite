 ...
 * 
 * Diese Tests überprüfen die Performance und Stabilität der SMB-Operationen
 * unter verschiedenen Belastungsszenarien:
 * - Massenoperationen (Bulk Operations)
 * - Langzeittests (Long-running Tests)  
 * - Speicher-Stresstests (Memory Stress)
 * - Nebenläufigkeits-Stresstests (Concurrency Stress)
...
public class SmbRepositoryPerformanceTest {
    ...
    public void testMemoryUsageStressTest() throws Exception {
        ...
        try {
            ...
            assertTrue("Memory usage should not increase excessively: " + memoryIncrease + " bytes", 
    ...
    public void testConcurrentOperationsPerformance() throws Exception {
        ...
        assertTrue("Concurrent operations should complete in reasonable time (< 20s)", 
                  executionTime < 20000);
    ...
    public void testLargeFilePerformance() throws Exception {
        ...
        for (int size : fileSizes) {
            ...
            // Verify integrity
            String copiedHash = calculateFileHash(copiedFile);
            assertEquals("Large file integrity should be maintained for size " + size, 
                        originalHash, copiedHash);
            ...
            if (size <= 100 * 1024) { // Files <= 100KB should be fast
                assertTrue("Small files should process quickly (< 1s) for size " + size, 
                          duration < 1000);
            } else if (size <= 1024 * 1024) { // Files <= 1MB should be reasonable
                assertTrue("Medium files should process reasonably (< 5s) for size " + size, 
                          duration < 5000);
            } else { // Large files can take longer but should still be reasonable
                assertTrue("Large files should process in reasonable time (< 15s) for size " + size, 
                          duration < 15000);
            }
        }
        ...
        assertTrue("Performance should scale reasonably with file size", 
                  time1MB < time100KB * 20);
    }
    ...
    public void testOperationTimeoutResilience() throws Exception {
        ...
        for (int i = 0; i < NUM_QUICK_OPERATIONS; i++) {
            ...
            try {
                ...
                assertTrue("Operation " + i + " should timeout quickly: " + duration + "ms", 
                          duration < 5000);
            }
        }
    ...
    public void testHashCalculationPerformance() throws Exception {
        ...
        for (int size : dataSizes) {
            ...
            // Performance assertions
            assertTrue("Hash calculation should be fast for size " + size + ": " + firstHashTime + "ms", 
                      firstHashTime < 1000);
            assertTrue("Second hash calculation should be similar: " + secondHashTime + "ms", 
                      secondHashTime < 1000);
            
            // Hash calculation time should not vary dramatically
            long timeDifference = Math.abs(firstHashTime - secondHashTime);
            assertTrue("Hash calculation time should be consistent (diff < 500ms): " + timeDifference, 
                      timeDifference < 500);
    ...
    public void testOperationCancellationSpeed() throws Exception {
        ...
        for (int i = 0; i < NUM_CANCELLATION_TESTS; i++) {
            ...
            assertTrue("Cancellation should be immediate: " + cancellationTime + "ms", 
                      cancellationTime < 100);
        }
        
        // Average cancellation time should be very fast
        double avgCancellationTime = cancellationTimes.stream()
                                                     .mapToLong(Long::longValue)
                                                     .average()
                                                     .orElse(0.0);
        assertTrue("Average cancellation time should be very fast: " + avgCancellationTime + "ms", 
    ...
    public void testAPIConsistencyUnderLoad() throws Exception {
        ...
        for (int i = 0; i < NUM_ITERATIONS; i++) {
            ...
            Date now = new Date();
            SmbFileItem fileItem = new SmbFileItem("test.txt", "/test.txt", 
                                                 SmbFileItem.Type.FILE, testFile.length(), now);
