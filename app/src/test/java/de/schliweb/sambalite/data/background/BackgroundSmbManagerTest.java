...
public class BackgroundSmbManagerTest {
   ...
   * BackgroundSmbManager still attempts to bind and, once connected, can execute a queued
   * operation.
   */
  @Test
  public void ensureServiceStartedAndBound_startNotAllowed_bindsAndExecutesQueuedOp()
      throws Exception {
    ...
    when(mockAppContext.bindService(
            any(Intent.class), connCaptor.capture(), eq(Context.BIND_AUTO_CREATE)))
        .thenReturn(true);
    ...
    class TestService extends SmbBackgroundService {
      @Override
      public void executeSmbOperation(
          String operationName, java.util.concurrent.Callable<Boolean> work) {
        try {
          work.call();
    ...
    CompletableFuture<Integer> fut =
        mgr.executeBackgroundOperation(
            "op1",
            "DOWNLOAD",
            callback -> {
              executed.set(true);
    ...
    capturedConn.onServiceConnected(
    ...
    verify(mockAppContext, times(1))
        .bindService(any(Intent.class), any(ServiceConnection.class), eq(Context.BIND_AUTO_CREATE));
  }

  /**
   * Verifies that requestCancelAllOperations falls back to bindService when starting is not
   * allowed.
   */
  @Test
  public void requestCancelAllOperations_startNotAllowed_fallsBackToBind() {
    ...
    verify(mockAppContext, org.mockito.Mockito.atLeastOnce())
        .bindService(
            intentCaptor.capture(), any(ServiceConnection.class), eq(Context.BIND_AUTO_CREATE));
    boolean hasCancel =
        intentCaptor.getAllValues().stream()
            .anyMatch(i -> SmbBackgroundService.ACTION_CANCEL.equals(i.getAction()));
