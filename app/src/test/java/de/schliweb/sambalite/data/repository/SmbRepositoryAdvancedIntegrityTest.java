 ...
 *
 * <p>These tests cover advanced scenarios and edge cases that could lead to data loss: - Concurrent
 * operations - Network interruption simulation - Partial transfer recovery - File corruption
 * detection - ZIP compression/decompression integrity - Memory pressure scenarios - Complex file
 * structures
 */
...
public class SmbRepositoryAdvancedIntegrityTest {
  ...
  public void testPartialTransferRecovery() throws Exception {
    ...
    try (FileInputStream fis = new FileInputStream(originalFile);
        FileOutputStream fos = new FileOutputStream(partialFile, true)) {
    ...
    assertEquals(
        "Resumed file should match original size", originalFile.length(), partialFile.length());
    String resumedHash = calculateFileHash(partialFile);
    assertEquals(
        "Resumed transfer should preserve complete data integrity", originalHash, resumedHash);
  }
  ...
  public void testZipIntegrity() throws Exception {
    ...
    testFiles.put(
        "text_file.txt", "This is a text file with\nmultiple lines\nand special chars: äöü");
    ...
    for (String fileName : testFiles.keySet()) {
      ...
      assertEquals(
          "ZIP/unzip should preserve data integrity for: " + fileName, originalHash, extractedHash);

      // Verify content matches
      String originalContent = testFiles.get(fileName);
      String extractedContent = readFileContent(extractedFile);
      assertEquals(
          "Content should match after ZIP/unzip: " + fileName, originalContent, extractedContent);
    }
  }
  ...
  public void testMemoryPressureIntegrity() throws Exception {
    ...
    for (int i = 0; i < largeFiles.size(); i++) {
      ...
      assertEquals(
          "Memory pressure should not cause data corruption: " + i,
          originalHashes.get(i),
          processedHash);
    }
  }
  ...
  public void testCorruptionDetection() throws Exception {
    ...
    assertFalse(
        "Corrupted file should not match original byte-by-byte",
        compareFilesByteByByte(originalFile, corruptedFile));
  }
  ...
  public void testComplexDirectoryStructure() throws Exception {
    ...
    for (String relativePath : fileContents.keySet()) {
      ...
      assertEquals(
          "File size should match: " + relativePath, originalFile.length(), copiedFile.length());

      String copiedHash = calculateFileHash(copiedFile);
      String originalHash = originalHashes.get(relativePath);
      assertEquals(
          "Directory structure transfer should preserve data: " + relativePath,
          originalHash,
          copiedHash);
    }
  }
  ...
  public void testExtremeFilenames() throws Exception {
    ...
    for (Map.Entry<String, String> entry : problematicFiles.entrySet()) {
      try {
        ...
        String copiedHash = calculateFileHash(copiedFile);
        assertEquals(
            "Extreme filename should not affect data integrity: " + entry.getKey(),
            hash,
            copiedHash);
  ...
  private void simulateTransferWithDelay(File source, File destination)
  ...
  private void copyFileWithSmallBuffer(File source, File destination, int bufferSize)
      throws IOException {
    try (FileInputStream fis = new FileInputStream(source);
        FileOutputStream fos = new FileOutputStream(destination)) {
  ...
  private void zipDirectoryRecursive(File source, String basePath, ZipOutputStream zos)
      throws IOException {
    if (source.isDirectory()) {
      ...
      String entryName =
          source.getAbsolutePath().substring(basePath.length() + 1).replace("\\", "/");
