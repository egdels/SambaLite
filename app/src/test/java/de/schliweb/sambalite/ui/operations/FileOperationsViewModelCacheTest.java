...
public class FileOperationsViewModelCacheTest {
  ...
  /** UT2.3 – Upload integrity check passes when remote size matches local size. */
  @Test
  public void performUpload_integrityPass_whenSizeMatches() throws Exception {
    File localFile = new File(cacheDir, "upload.txt");
    writeBytes(localFile, 750L);

    // Mock upload success
    doNothing()
        .when(smbRepository)
        .uploadFileWithProgress(eq(connection), eq(localFile), anyString(), any());

    // Mock remote file size matching local
    when(smbRepository.getRemoteFileSize(eq(connection), anyString())).thenReturn(750L);
    ...
    viewModel.performUpload(
        ...
        new FileOperationCallbacks.UploadCallback() {
          @Override
          public void onResult(boolean ok, @androidx.annotation.NonNull String msg) {
            success.set(ok);
            message.set(msg);
            latch.countDown();
          }
          ...
          @Override
          public void onProgress(@androidx.annotation.NonNull String status, int percentage) {}
        });

    // Idle the looper repeatedly to allow executor + mainHandler.post to complete
    for (int i = 0; i < 50 && latch.getCount() > 0; i++) {
      ShadowLooper.idleMainLooper();
      Thread.sleep(100);
    }

    assertTrue("Upload should succeed", success.get());
  }

  /** UT2.4 – Upload integrity check fails when remote size differs from local size. */
  @Test
  public void performUpload_integrityFail_whenSizeMismatch() throws Exception {
    File localFile = new File(cacheDir, "upload.txt");
    writeBytes(localFile, 750L);
    ...
    // Mock remote file size NOT matching local (incomplete upload)
    when(smbRepository.getRemoteFileSize(eq(connection), anyString())).thenReturn(400L);
    ...
    assertFalse("Upload should fail due to integrity mismatch", success.get());
  }
  ...
  // ========== getRemoteFileSize Tests ==========

  /**
   * UT6.1 – getRemoteFileSize returns correct size for existing file. Verified via upload integrity
   * check: if getRemoteFileSize returns matching size, upload succeeds.
   */
  @Test
  public void getRemoteFileSize_usedInUploadIntegrityCheck_matchingSize() throws Exception {
    File localFile = new File(cacheDir, "check.txt");
    writeBytes(localFile, 4096L);

    doNothing()
        .when(smbRepository)
        .uploadFileWithProgress(eq(connection), eq(localFile), anyString(), any());
    when(smbRepository.getRemoteFileSize(eq(connection), anyString())).thenReturn(4096L);
    ...
    verify(smbRepository).getRemoteFileSize(eq(connection), eq("/share/check.txt"));
    assertTrue("Upload should succeed when remote size matches", success.get());
  }

  /**
   * UT6.2 – getRemoteFileSize returns -1 (file not found). Upload should still succeed since -1
   * means "unable to verify" and is not treated as a mismatch.
   */
  @Test
  public void getRemoteFileSize_returnsMinusOne_uploadStillSucceeds() throws Exception {
    File localFile = new File(cacheDir, "check2.txt");
    writeBytes(localFile, 500L);

    doNothing()
        .when(smbRepository)
        .uploadFileWithProgress(eq(connection), eq(localFile), anyString(), any());
    when(smbRepository.getRemoteFileSize(eq(connection), anyString())).thenReturn(-1L);
    ...
    assertTrue("Upload should succeed when remote size is -1 (unverifiable)", success.get());
  }

  /** UT6.3 – getRemoteFileSize returns mismatching size. Upload should fail. */
  @Test
  public void getRemoteFileSize_mismatch_uploadFails() throws Exception {
    File localFile = new File(cacheDir, "check3.txt");
    writeBytes(localFile, 1000L);

    doNothing()
        .when(smbRepository)
        .uploadFileWithProgress(eq(connection), eq(localFile), anyString(), any());
    when(smbRepository.getRemoteFileSize(eq(connection), anyString())).thenReturn(500L);
    ...
    assertFalse("Upload should fail when remote size mismatches", success.get());
  }
